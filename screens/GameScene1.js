...
export class GameScene1 {
  ...
  start() {
    ...
    const hero1 = new character("Warrior", 10, 15, "assets/characters/fire_knight/spritesheets/fire_spritesheet.png", [
      skilldatabase["Slash"], skilldatabase["Double"], skilldatabase["Triple"], skilldatabase["Ultimate"]
    ], "assets/characters/fire_knight/fire_knight.png");

    const hero2 = new character("Mage", 70, 25, "assets/characters/water_priestess/water_spritesheet.png", [
      skilldatabase["Fireball"], skilldatabase["Ice"], skilldatabase["Thunder"], skilldatabase["Meteor"]
    ], "assets/characters/water_priestess/water_priestess.png");

    const hero3 = new character("Guardian", 70, 10, "assets/characters/wind_hashashin/wind_spritesheet.png", [
      skilldatabase["Shield"], skilldatabase["Wind"], skilldatabase["Storm"], skilldatabase["Tornado"]
    ], "assets/characters/wind_hashashin/wind_hashashin.png");

    const boss = new character("Demon Lord", 209, 70, "assets/characters/demon_slime/spritesheets/demon_slime_spritesheet.png", []);
...
boss.animationState = "idle";
  ...
  if (typeof this.onExit === "function") {
    ...
    Promise.all([
      ...
      hero1.setAnimator(new SpriteAnimator(img1, HERO_ANIMATIONS.fire_knight, 3));
      hero2.setAnimator(new SpriteAnimator(img2, HERO_ANIMATIONS.water_priestess, 3));
      hero3.setAnimator(new SpriteAnimator(img3, HERO_ANIMATIONS.wind_hashashin, 3));
      boss.setAnimator(new SpriteAnimator(imgBoss, HERO_ANIMATIONS.demon_slime, 3));
    ...
    this._playerActionHandler = (e) => {
      ...
      if (ui.commandPanel) ui.commandPanel.innerHTML = '';
    ...
    this._playerSkillHandler = (e) => {
      ...
      if (ui.skillMenuElement) ui.skillMenuElement.innerHTML = '';
      if (ui.commandPanel) ui.commandPanel.innerHTML = '';
  ...
  startHPLoop(ui) {
    const hpLoop = () => {
      ui.updateHPBars();
      this._animFrameId = requestAnimationFrame(hpLoop);
    };
    hpLoop();
  }
